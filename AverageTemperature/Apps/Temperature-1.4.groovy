...
private String getChildDriverType() {
    ...
    return shortVersion ? "Temperature-${shortVersion}" : null
}
...
private List<String> temperatureUnitDisplayKeys() {
    return temperatureUnitDisplayOptions().keySet() as List<String>
}

private String defaultTemperatureDisplayUnit() {
...
def refresh() {
    List<BigDecimal> values = getValidTemperatureValues()
    if (!values) {
        log.warn 'No valid temperature values found from selected sensors.'
        return
    }
...
private String getStableChildDni() {
    return "Temperature-${app?.id ?: 'pending'}"
}

private String getLegacyVersionedChildDni() {
    String version = getParentVersionValue()
    return (version && app?.id) ? "Temperature-${version}-${app.id}" : null
}
...
private Boolean isManagedChildDni(String dni) {
    ...
    return dni.startsWith('Temperature-') && dni.endsWith("-${app.id}")
}
...
private List getValidTemperatureDevices() {
    return getSelectedDevices().findAll { dev ->
        try {
            def v = dev.currentValue('temperature')
            return v != null && v.toString() != ''
        } catch (Exception e) {
            logDebug("Skipping ${dev?.displayName}: unable to read temperature: ${e.message}")
            return false
        }
    }
}

private List<BigDecimal> getValidTemperatureValues() {
    List<BigDecimal> values = []

    getValidTemperatureDevices().each { dev ->
        try {
            Object raw = dev.currentValue('temperature')
...
private void clearTrendStateAndAttribute() {
    state.remove('temperatureHistory')
    def child = getManagedChildDevice()
    child?.clearTrend()
}
...
private Map updateTrendAndGetValues(BigDecimal roundedTemperature) {
    if (selectedTrendDisplay() == 'off') {
        clearTrendStateAndAttribute()
    ...
    if (lastValue == null || lastValue.compareTo(roundedTemperature) != 0) {
        history << [ts: now(), value: roundedTemperature.toString()]
        while (history.size() > configuredTrendDepth()) {
            history.remove(0)
        }
        saveHistoryEntries(history)
...
private List<Map> getHistoryEntries() {
    List raw = state?.temperatureHistory instanceof List ? state.temperatureHistory as List : []
    List<Map> history = []
...
private void saveHistoryEntries(List<Map> history) {
    state.temperatureHistory = history.collect { entry ->
