definition(
    ...
    importUrl: 'https://raw.githubusercontent.com/vinnyw/hubitat/master/AverageAirQuality/Apps/AirQuality-1.0.groovy',
...
private String getChildDriverType() {
    ...
    return shortVersion ? "AirQuality-${shortVersion}" : null
}
    ...
    dynamicPage(name: 'mainPage', install: true, uninstall: true) {
        ...
        section() {
            ...
            input(
                ...
                title: 'TVOC Air Quality sensors',
                multiple: true,
                required: true,
                showFilter: true,
                submitOnChange: true
        ...
        section(
            ...
            paragraph 'Optional tuning settings for display units and trend analysis. Air Quality Index is always published as an integer.'

            input(
                name: 'unitDisplay',
                type: 'enum',
                title: 'Unit',
                options: airQualityUnitDisplayOptions(),
...
private void applyDefaultSettings() {
    ensureEnumSetting('unitDisplay', airQualityUnitDisplayKeys(), 'none')
...
private String selectedTrendDisplay() {
    String mode = settings?.trendDisplay?.toString() ?: 'off'
...
private String selectedUnitDisplaySetting() {
    String unit = settings?.unitDisplay?.toString() ?: 'none'
    return airQualityUnitDisplayKeys().contains(unit) ? unit : 'none'
}
...
private Map<String, String> airQualityUnitDisplayOptions() {
    return [
        'none' : 'none',
        'mg/m³': 'mg/m³',
        'ppb'  : 'ppb'
...
def initialize() {
    ...
    logDebug("Selected airQualityIndex devices after filtering: ${configured*.displayName}")

    if (configured) {
        subscribe(configured, 'airQualityIndex', 'airQualityHandler', [filterEvents: true])
        logDebug("Subscribed to airQualityIndex events for ${configured*.displayName}")
    } else {
        log.warn 'No valid devices selected for airQualityIndex averaging.'
    }
...
def airQualityHandler(evt) {
    ...
    logDebug("AirQuality event from ${evt.device?.displayName}: ${evt.value}")
    refresh()
}
...
def refresh() {
    List<BigDecimal> values = getValidAirQualityValues()
    if (!values) {
        log.warn 'No valid air quality index values found from selected sensors.'
        return
    }

    BigDecimal average = calculateAverage(values)
    BigDecimal rounded = average.setScale(0, RoundingMode.HALF_UP)
    ...
    logDebug("Air Quality Index values=${values}, average=${average}, rounded=${rounded}, trend=${trendData?.trend}, trendDisplay=${trendData?.trendDisplay}")
    String canonicalAirQualityIndex = rounded.toPlainString()
    String selectedDisplayUnit = selectedUnitDisplay()
    String airQualityDisplay = selectedDisplayUnit == 'none'
        ? canonicalAirQualityIndex
        : "${canonicalAirQualityIndex} ${selectedDisplayUnit}"
...
private List getSelectedDevices() {
    return normalizeSelectedSensorList().findAll { dev ->
        isEligibleAirQualitySourceDevice(dev)
    }
}
...
private Boolean isEligibleAirQualitySourceDevice(dev) {
    if (!dev) return false

    if (isAverageAirQualityVirtualOutputDevice(dev)) {
        return false
    }

    try {
        if (dev.hasCapability('AirQuality') != true) {
            return false
        }
    } catch (Exception e) {
        logDebug("Skipping ${dev?.displayName}: unable to verify AirQuality capability: ${e.message}")
        return false
    }

    try {
        if (dev.hasAttribute('airQualityIndex') != true) {
            return false
        }
    } catch (Exception e) {
        logDebug("Skipping ${dev?.displayName}: unable to verify airQualityIndex attribute: ${e.message}")
        return false
    }
...
private Boolean isAverageAirQualityVirtualOutputDevice(dev) {
    if (!dev) return false
    ...
    try {
        return dev.getDataValue('averageAirQualityVirtualDevice') == 'true'
    } catch (Exception e) {
        logDebug("Unable to read Average Air Quality marker from ${dev?.displayName}: ${e.message}")
        return false
    }
}

private List getValidAirQualityDevices() {
    return getSelectedDevices().findAll { dev ->
        try {
            def v = dev.currentValue('airQualityIndex')
            return v != null && v.toString() != ''
        } catch (Exception e) {
            logDebug("Skipping ${dev?.displayName}: unable to read airQualityIndex: ${e.message}")
            return false
        }
    }
}

private List<BigDecimal> getValidAirQualityValues() {
    List<BigDecimal> values = []

    getValidAirQualityDevices().each { dev ->
        try {
            Object raw = dev.currentValue('airQualityIndex')
...
private void clearTrendStateAndAttribute() {
    state.remove('airQualityHistory')
    def child = getManagedChildDevice()
    child?.clearTrend()
}
...
private Map updateTrendAndGetValues(BigDecimal roundedAirQualityIndex) {
    if (selectedTrendDisplay() == 'off') {
        clearTrendStateAndAttribute()
    ...
    if (lastValue == null || lastValue.compareTo(roundedAirQualityIndex) != 0) {
        history << [ts: now(), value: roundedAirQualityIndex.toString()]
        while (history.size() > configuredTrendDepth()) {
            history.remove(0)
        }
        saveHistoryEntries(history)
...
private List<Map> getHistoryEntries() {
    List raw = state?.airQualityHistory instanceof List ? state.airQualityHistory as List : []
    List<Map> history = []
...
private void saveHistoryEntries(List<Map> history) {
    state.airQualityHistory = history.collect { entry ->
