/* groovylint-disable  NglParseError, CompileStatic, DuplicateMapLiteral, DuplicateNumberLiteral, DuplicateStringLiteral, ImplicitClosureParameter, LineLength, MethodCount, PublicMethodsBeforeNonPublicMethods, UnnecessaryGetter, UnusedImport */
/**
 *  Aqara TVOC Air Quality Monitor - Device Driver for Hubitat Elevation
 *
...
metadata {
    definition(
        name: 'Aqara TVOC Air Quality Monitor (fixed)',
...
void parseXiaomiClusterAirQualityLib(final Map descMap) {
    final Integer attr = descMap.attrInt as Integer

    // PATCH: silently ignore known noisy Aqara attribute 0x0129
    if (attr == 0x0129) {
        return
    }
    ...
    // Optimization: ignore undocumented Xiaomi attributes except the 0x00F7 tag report
    if ((descMap.attrInt as Integer) != 0x00F7) {
        Boolean handled = processClusterAttributeFromDeviceProfile(descMap)
        if (handled != true) {
            logTrace "ignored Xiaomi 0xFCC0 attribute 0x${descMap.attrId} value ${descMap.value}"
        }
        return
    }
    Boolean result
...
String airQualityIndexToLevel(final Integer index) {
    ...
    else if (index <= 300)   { level = 'Good' }
    else if (index <= 500)   { level = 'Moderate' }
    else if (index <= 1000)  { level = 'Unhealthy for Sensitive Groups' }
    else if (index <= 3000)  { level = 'Unhealthy' }
    else if (index <= 5000)  { level = 'Very Unhealthy' }
...
void aqaraBlackMagic() { // library marker kkossev.commonLib, line 2159
    ...
    if (isAqaraTVOC_OLD() || isAqaraTRV_OLD()) { // library marker kkossev.commonLib, line 2161
        logDebug 'aqaraBlackMagic(): skipping unsupported he raw preamble; sending only validated commands'
        cmds += ['delay 200',] // library marker kkossev.commonLib, line 2162
...
private boolean isSafeZigbeeCommand(final String cmd) {
    if (cmd == null) { return false }
    final String trimmed = cmd.trim()
    if (trimmed == '') { return false }
    if (trimmed.startsWith('delay ')) { return true }
    if (trimmed.startsWith('zdo bind ') || trimmed.startsWith('zdo unbind ')) { return true }
    if (trimmed.startsWith('he raw ')) { return false }
    if (trimmed.startsWith('he rattr ') || trimmed.startsWith('he wattr ') || trimmed.startsWith('he cr ') || trimmed.startsWith('he cmd ')) { return true }
    return false
}

private List<String> sanitizeZigbeeCommands(final List<String> cmdList) {
    List<String> safeCmds = []
    cmdList?.each { String it ->
        if (isSafeZigbeeCommand(it)) {
            safeCmds << it
        }
        else if (it != null && it != 'null') {
            logWarn "skipping unsupported Zigbee command: ${it}"
        }
    }
    return safeCmds
}
...
void sendZigbeeCommands(List<String> cmd) { // library marker kkossev.commonLib, line 2607
    ...
    List<String> safeCmds = sanitizeZigbeeCommands(cmd)
    if (safeCmds == null || safeCmds == []) {
        logWarn 'sendZigbeeCommands: all commands were rejected by validation'
        return
    }
    logDebug "sendZigbeeCommands(cmd=$safeCmds)" // library marker kkossev.commonLib, line 2612
    hubitat.device.HubMultiAction allActions = new hubitat.device.HubMultiAction() // library marker kkossev.commonLib, line 2613
    safeCmds.each { String it -> // library marker kkossev.commonLib, line 2614
